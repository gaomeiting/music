...
<div class="music-list" ref="musicList">
 	...
 	<scroll :data="songs" :listen-scroll="listenScroll" :probe-type="probeType" @scroll="scroll" class="list" ref="list">
 		<div class="song-list-wrapper">
 			<song-list :songs="songs" :topList="topList" @selectSong="selectSong"></song-list>
 		</div>
...
watch: {
	scrollY(newY) {
		let maxHeight= this.imageHeight-RESERVED_HEIGHT
		let translateY=Math.max(-maxHeight, newY)
		let scale=0
		let blur=0
		this.$refs.bgImage.style.zIndex=0

		if(newY>0) {
			scale=newY /  maxHeight + 1
			this.$refs.bgImage.style['transform']=`scale(${scale})`
			this.$refs.bgImage.style.zIndex=10
		}
		else {
			let percent= newY /  maxHeight 
